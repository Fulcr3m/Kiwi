...
HttpConnection::~HttpConnection() {
    KIWI_LOG_DEBUG(g_logger) << "HttpConnection::~HttpConnection";
}
...
HttpResponse::ptr HttpConnection::recvResponse() {
    ...
    do {
        ...
        len += offset;
        data[len] = '\0';
        size_t nparse = parser->execute(data, len, false);  //执行后data最开始是未解析的(memmove)
        if(parser->hasError()) {
            close();
            return nullptr;
        }
    ...
    if(client_parser.chunked) {      //分块传输
        ...
        do {
            ...
            KIWI_LOG_INFO(g_logger) << "content_len=" << client_parser.content_len;
...
HttpConnection::ptr HttpConnectionPool::getConnection() {
    uint64_t now_ms = kiwi::GetCurrentMS();
    ...
    if(!ptr) {
        ...
        if(!addr) {
            KIWI_LOG_ERROR(g_logger) << "get addr fail: " << m_host;
            return nullptr;
        }
        addr->setPort(m_port);
        Socket::ptr sock = Socket::CreateTCP(addr);
        if(!sock) {
            KIWI_LOG_ERROR(g_logger) << "create sock fail: " << *addr;
            return nullptr;
        }
        if(!sock->connect(addr)) {
            KIWI_LOG_ERROR(g_logger) << "sock connect fail: " << *addr;
            return nullptr;
        }
...
void HttpConnectionPool::ReleasePtr(HttpConnection* ptr, HttpConnectionPool* pool) {
    ++ptr->m_request;
    if(!ptr->isConnected()
            || ((ptr->m_createTime + pool->m_maxAliveTime) >= kiwi::GetCurrentMS())
