#ifndef __KIWI_CONFIG_H__
#define __KIWI_CONFIG_H__
...
public:
    ...
    std::string toString() override {
        try {
            ...
            KIWI_LOG_ERROR(KIWI_LOG_ROOT()) << "ConfigVar::toString exception"
...
public:
    ...
    static typename ConfigVar<T>::ptr Lookup(const std::string& name,
        ...
        if(it != GetDatas().end()) {
            ...
            if(tmp) {
                KIWI_LOG_INFO(KIWI_LOG_ROOT()) << "Lookup name=" << name << " exists";
                return tmp;
            } else {
                KIWI_LOG_ERROR(KIWI_LOG_ROOT()) << "Lookup name=" << name << " exists but type not "
        ...
        if(name.find_first_not_of("abcdefghikjlmnopqrstuvwxyz._012345678")
                != std::string::npos) {
            KIWI_LOG_ERROR(KIWI_LOG_ROOT()) << "Lookup name invalid " << name;
